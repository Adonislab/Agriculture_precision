...
function setBotResponse(response) {
    ...
    setTimeout(function() {
        ...
        if (response.length < 1) {
            ...
            var fallbackMsg = "J'étais un peu pris, vous pouvez recommancer !!!";

            var BotResponse = '<img class="botAvatar" src="{% static "bot/img/sara_avatar.png" %}"/> <p class="botMsg">' + fallbackMsg + '</p><div class="clearfix"></div>';
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("text")) {
                    var BotResponse = '<img class="botAvatar" src="{% static "bot/img/sara_avatar.png" %}"/> <p class="botMsg">' + response[i].text + '</p><div class="clearfix"></div>';
                    $(BotResponse).appendTo(".chats").hide().fadeIn(1000);
                }
